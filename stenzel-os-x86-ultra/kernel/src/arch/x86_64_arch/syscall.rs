...
#[repr(C)]
pub struct CpuLocal {
    /// Topo do kernel stack do *task atual* (usado pelo syscall entry).
    pub kernel_stack_top: u64,
    /// Slot temporário para salvar o RSP do usuário durante o entry.
    pub user_rsp_tmp: u64,
}

/// Wrapper para permitir CpuLocal em static
/// Seguro pois só há uma CPU e acessamos via GS base
#[repr(transparent)]
struct SyncCpuLocal(UnsafeCell<CpuLocal>);
unsafe impl Sync for SyncCpuLocal {}

static CPU_LOCAL: SyncCpuLocal = SyncCpuLocal(UnsafeCell::new(CpuLocal {
    kernel_stack_top: 0,
    user_rsp_tmp: 0,
}));
...
/// Atualiza o stack usado pelo `syscall` para o task atual.
pub unsafe fn set_kernel_stack_top(top: u64) {
    crate::kprintln!("syscall: set_kernel_stack_top({:#x})", top);
    (*CPU_LOCAL.0.get()).kernel_stack_top = top;
}
...
extern "C" fn syscall_dispatch(frame: &mut SyscallFrame) -> u64 {
    ...
    let result: i64 = match frame.rax {
        ...
        nr::PSELECT6 => crate::syscall::sys_select(frame.rdi as i32, frame.rsi, frame.rdx, frame.r10, frame.r8),
        nr::PPOLL => crate::syscall::sys_poll(frame.rdi, frame.rsi, frame.rdx as i32),
        // Thread syscalls
